<template>
  <q-page class="q-pa-md storefront-page">
    <!-- ACCESS DENIED STATE -->
    <div v-if="accessDenied" class="column items-center justify-center error-container text-center q-pa-xl">
    ...
    <!-- NOT FOUND STATE -->
    <div v-else-if="notFound" class="column items-center justify-center error-container text-center q-pa-xl">
    ...
    <div v-else>
          ...
          <div class="row items-center justify-between q-col-gutter-sm">
            <div class="col">
              <div class="row items-center q-gutter-sm">
                ...
                <div>
                  <h1 class="text-h5 text-weight-bold q-my-none">{{ shopStorefrontStore.shopDetails?.name }}</h1>
                  <div class="row items-center q-gutter-xs text-caption text-grey-7 q-mt-xs">
                    <q-badge :color="getShopTypeColor(shopStorefrontStore.shopDetails?.shop_type)" text-color="white" class="q-mr-xs">
                      {{ getShopTypeLabel(shopStorefrontStore.shopDetails?.shop_type) }}
                    </q-badge>
                    <span>•</span>
                    <span>Order Mode: {{ getOrderModeLabel(shopStorefrontStore.shopDetails?.order_mode) }}</span>
                  </div>
                </div>
              </div>
            </div>
      ...
      <q-infinite-scroll ref="infiniteScrollRef" :offset="250" @load="onLoadMore">
            ...
            <q-card flat bordered class="product-card full-height column justify-between">
              ...
              <q-card-section class="q-pa-sm q-pt-none border-top">
                <div class="row items-center justify-between q-mt-sm">
                  ...
                  <div>
                    <div v-if="shopStorefrontStore.permissions?.see_price" class="column">
                      ...
                      <div v-if="item.minimum_sell_price_amount" class="text-caption text-grey-6">
                        Min Sell: {{ item.minimum_sell_price_currency_symbol || '£' }}{{ Number(item.minimum_sell_price_amount).toFixed(2) }}
                      </div>
                    </div>
                  ...
                  <div class="text-right">
                    <div v-if="shopStorefrontStore.permissions?.can_view_quantity" class="column items-end">
              ...
              <q-card-actions align="right" class="q-pa-sm q-pt-none">
                <q-btn
                  ...
                  :disabled="!shopStorefrontStore.permissions?.can_add_to_cart || (item.available_units !== null && item.available_units <= 0)"
        ...
        <div v-else-if="!shopStorefrontStore.loading" class="column items-center justify-center empty-state q-pa-xl text-center">
          ...
          <p class="text-body2 text-grey-6 q-mt-sm">There are no products matching the current criteria.</p>
        </div>
    ...
    <FilterSidebar v-model="filterDrawerOpen" title="Filters">
      <div class="q-pa-md">
        ...
        <div class="row q-gutter-sm justify-end q-mt-md">
          ...
          <q-btn unelevated no-caps label="Apply" color="primary" class="pill-btn" @click="filterDrawerOpen = false" />
        </div>
      </div>
    </FilterSidebar>
  </q-page>
...
.product-card:hover {
  ...
  box-shadow: 0 10px 20px rgba(34, 56, 101, 0.06), 0 2px 6px rgba(34, 56, 101, 0.04);
}

...
): Promise<CustomerGroupShopProfile | null> => {
  const { data, error } = await supabase
...
const upsertProfile = async (payload: UpsertProfilePayload): Promise<CustomerGroupShopProfile> => {
  const { data, error } = await supabase.rpc('upsert_customer_group_shop_profile', {
    ...
    p_default_can_see_buy_price: payload.default_can_see_buy_price,
    p_default_can_see_sell_price: payload.default_can_see_sell_price,
...
): Promise<ShopCustomerGroupAccess> => {

...
const upsertListing = async (payload: UpsertListingPayload): Promise<ShopServiceResult<ShopProductListing>> => {
  try {
...
const listCandidateAllocations = async (tenantId: number, shopId: number): Promise<ShopServiceResult<CandidateAllocation[]>> => {
  try {

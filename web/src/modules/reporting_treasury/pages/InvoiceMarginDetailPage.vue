      ...
      <q-card flat bordered class="q-pa-md bg-white">
        <div class="row q-col-gutter-md">
          ...
          <div class="col-12 col-sm-6 col-md-3">
            ...
            <div class="text-subtitle1 text-weight-bold">{{ invoice.recipient_name || 'Walk-in / Direct' }}</div>
            <div v-if="invoice.recipient_phone" class="text-caption text-grey-6">{{ invoice.recipient_phone }}</div>
          </div>
          <div class="col-12 col-sm-6 col-md-3">
          ...
          <div class="col-12 col-sm-6 col-md-3">
            ...
            <div class="row q-gutter-xs items-center q-mt-xs">
              <q-chip dense square color="blue-1" text-color="blue-9" class="text-weight-bold text-uppercase text-xs m-0">
                {{ invoice.invoice_type }}
              </q-chip>
              <q-chip dense square color="green-1" text-color="green-9" class="text-weight-bold text-uppercase text-xs m-0">
                {{ invoice.invoice_status }}
              </q-chip>
              <q-chip dense square color="orange-1" text-color="orange-9" class="text-weight-bold text-uppercase text-xs m-0">
          ...
          >
            ...
            <template #body-cell-line_discount_amount="props">
              <q-td :props="props" class="text-right text-negative">
                {{ props.row.line_discount_amount > 0 ? `-${formatAmountBdt(props.row.line_discount_amount)}` : '-' }}
              </q-td>
            </template>

            <template #body-cell-line_total_amount="props">
              <q-td :props="props" class="text-right text-weight-bold">
                {{ formatAmountBdt((props.row.sell_price_amount * props.row.quantity) - props.row.line_discount_amount) }}
              </q-td>
            </template>
            ...
            <template #body-cell-margin_pct="props">
              <q-td :props="props" class="text-right text-weight-bold" :class="getMarginClass(props.row.line_margin, props.row.sell_price_amount * props.row.quantity)">
                {{ formatPercent(props.row.line_margin, props.row.sell_price_amount * props.row.quantity) }}
              </q-td>
            </template>
          </q-table>
        </TreasuryTableWrap>
...
const lineColumns: QTableColumn[] = [
  { name: 'name_snapshot', label: 'Product / Item Name', field: 'name_snapshot', align: 'left', sortable: true },
  { name: 'quantity', label: 'Qty', field: 'quantity', align: 'right', sortable: true },
  { name: 'unit_cost_price', label: 'Landed Unit Cost', field: 'unit_cost_price', align: 'right', sortable: true },
  { name: 'total_cost', label: 'Total Landed Cost', field: 'id', align: 'right' },
  { name: 'sell_price_amount', label: 'Unit Sell Price', field: 'sell_price_amount', align: 'right', sortable: true },
  { name: 'line_discount_amount', label: 'Line Discount', field: 'line_discount_amount', align: 'right', sortable: true },
...
const returnColumns: QTableColumn[] = [
  ...
  { name: 'total_cost_reversal', label: 'Landed Cost Reversal', field: 'invoice_item_id', align: 'right' },
  { name: 'return_accounting_amount', label: 'Refund/Reversal Amt', field: 'return_accounting_amount', align: 'right' },
...
const loadDetail = async () => {
  ...
  try {
    ...
    $q.notify({ type: 'negative', message: `Failed to load invoice margin detail: ${err.message}` })
  } finally {
    loading.value = false
  }
}

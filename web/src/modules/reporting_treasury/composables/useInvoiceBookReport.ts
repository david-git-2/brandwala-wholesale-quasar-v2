...
export function useInvoiceBookReport() {
  ...
  watch([startDate, endDate, invoiceType, paymentStatus], () => {
    page.value = 1;
  });
  ...
  const reportQuery = useQuery({
    queryKey: computed(() =>
      tenantId.value
        ? financeReportQueryKeys.invoiceBook(tenantId.value, queryParams.value)
        : financeReportQueryKeys.root,
    ),
    queryFn: () => financeReportsRepository.fetchInvoiceBookReport(queryParams.value),
  ...
  return {
    ...
    exportCsv: () => {
      if (reportQuery.data.value) financeReportsRepository.exportInvoiceBookCsv(reportQuery.data.value);
    },

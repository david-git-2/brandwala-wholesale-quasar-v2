...
export function useInvoiceProfitReport() {
  ...
  const reportQuery = useQuery({
    queryKey: computed(() =>
      tenantId.value
        ? financeReportQueryKeys.invoiceProfit(tenantId.value, listParams.value)
        : financeReportQueryKeys.root,
    ),
    queryFn: () => financeReportsRepository.fetchInvoiceProfitReport(listParams.value),
  ...
  const detailQuery = useQuery({
    queryKey: computed(() =>
      tenantId.value && selectedInvoiceId.value
        ? financeReportQueryKeys.invoiceProfit(tenantId.value, {
            ...listParams.value,
    ...
    queryFn: () =>
      financeReportsRepository.fetchInvoiceProfitReport({
        ...listParams.value,
        invoiceId: selectedInvoiceId.value,
        page: 1,
        pageSize: 1,
      }),
    enabled: computed(() => Boolean(tenantId.value && selectedInvoiceId.value)),
    staleTime: 30_000,
  });
  ...
  return {
    ...
    detailLines: computed(() => detailQuery.data.value?.lines ?? []),
    ...
    exportCsv: () => {
      if (reportQuery.data.value) financeReportsRepository.exportInvoiceProfitCsv(reportQuery.data.value);
    },

import { ref, computed, watch } from 'vue';
import { useQuery } from '@tanstack/vue-query';
import { useAuthStore } from 'src/modules/auth/stores/authStore';
import { financeReportsRepository } from '../repositories/financeReportsRepository';
import { financeReportQueryKeys } from '../shared/queryKeys';
import { applyDatePreset, type DatePreset } from '../utils/datePresets';

export function useCourierCodReport() {
  const authStore = useAuthStore();
  const tenantId = computed(() => authStore.selectedTenant?.id ?? null);
  const preset = ref<DatePreset>('month');
  const startDate = ref<string | null>(null);
  const endDate = ref<string | null>(null);
  const selectedCourierId = ref<string | null>(null);
  const page = ref(1);
  const pageSize = ref(50);

  const applyPreset = (val: DatePreset) => {
    preset.value = val;
    if (val === 'custom') return;
    const range = applyDatePreset(val);
    startDate.value = range.startDate;
    endDate.value = range.endDate;
    page.value = 1;
  };
  applyPreset('month');

  const listParams = computed(() => ({
    tenantId: tenantId.value || 0,
    startDate: startDate.value,
    endDate: endDate.value,
    courierServiceId: null as string | null,
    page: page.value,
    pageSize: pageSize.value,
  }));

  const reportQuery = useQuery({
    queryKey: computed(() =>
      tenantId.value
        ? financeReportQueryKeys.courierCod(tenantId.value, listParams.value)
        : financeReportQueryKeys.root,
    ),
    queryFn: () => financeReportsRepository.fetchCourierCodReport(listParams.value),
    enabled: computed(() => Boolean(tenantId.value && tenantId.value > 0)),
    staleTime: 30_000,
  });

  const detailQuery = useQuery({
    queryKey: computed(() =>
      tenantId.value && selectedCourierId.value
        ? financeReportQueryKeys.courierCod(tenantId.value, {
            ...listParams.value,
            courierServiceId: selectedCourierId.value,
          })
        : financeReportQueryKeys.root,
    ),
    queryFn: () =>
      financeReportsRepository.fetchCourierCodReport({
        ...listParams.value,
        courierServiceId: selectedCourierId.value,
      }),
    enabled: computed(() => Boolean(tenantId.value && selectedCourierId.value)),
    staleTime: 30_000,
  });

  return {
    totals: computed(() => reportQuery.data.value?.totals ?? null),
    rows: computed(() => reportQuery.data.value?.rows ?? []),
    orders: computed(() => detailQuery.data.value?.orders ?? []),
    ordersLoading: detailQuery.isLoading,
    isLoading: reportQuery.isLoading,
    error: computed(() => (reportQuery.error.value as Error | null)?.message ?? null),
    preset,
    startDate,
    endDate,
    selectedCourierId,
    setPreset: applyPreset,
    exportCsv: () => {
      if (reportQuery.data.value) financeReportsRepository.exportCourierCodCsv(reportQuery.data.value);
    },
    refetch: () => reportQuery.refetch(),
  };
}

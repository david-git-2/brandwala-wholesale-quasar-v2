...
const listInvestorProfiles = async (
  tenantId: number,
): Promise<CapitalServiceResult<InvestorBalance[]>> => {
  try {
...
const deleteInvestorProfile = async (
  id: number,
  tenantId: number,
): Promise<CapitalServiceResult<void>> => {
  try {
...
const listTransactions = async (
  ...
  try {
    const data = await investorCapitalRepository.listTransactions(
      tenantId,
      limit,
      offset,
      investorId,
...
const listShipmentInvestmentsByShipment = async (
  ...
  try {
    const data = await investorCapitalRepository.listShipmentInvestmentsByShipment(
      tenantId,
      globalShipmentId,
    );
    return { success: true, data };
...
const upsertShipmentInvestment = async (
  payload: any,
): Promise<CapitalServiceResult<ShipmentInvestment>> => {
  try {
...
const refreshShipmentInvestorProfits = async (
  globalShipmentId: number,
): Promise<CapitalServiceResult<void>> => {
  try {
...
const getInvestorCapitalReport = async (
  ...
  try {
    const data = await investorCapitalRepository.getInvestorCapitalReport(
      tenantId,
      investorId,
      startDate,
      endDate,

...
export const useInvestorCapitalStore = defineStore('investorCapital', {
  ...
  actions: {
    ...
    async deleteInvestor(payload: InvestorDeleteInput) {
      ...
      try {
        const result = await investorCapitalService.deleteInvestorProfile(
          payload.id,
          payload.tenant_id,
        );
    ...
    async fetchTransactionsByTenant(
      ...
      try {
        const result = await investorCapitalService.listTransactions(
          tenantId,
          limit,
          offset,
          investorId,
    ...
    async deleteShipmentInvestment(
      payload: ShipmentInvestmentDeleteInput & { global_shipment_id: number },
    ) {
      this.saving = true;
      this.error = null;

      try {
        const result = await investorCapitalService.deleteShipmentInvestment(
          payload.id,
          payload.tenant_id,
        );
    ...
    async refreshProfits(tenantId: number, globalShipmentId: number) {
      ...
      try {
        const result =
          await investorCapitalService.refreshShipmentInvestorProfits(globalShipmentId);

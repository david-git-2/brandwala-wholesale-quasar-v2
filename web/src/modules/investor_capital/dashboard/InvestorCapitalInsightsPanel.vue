<template>
  ...
  <DashboardPulseCard v-else title="Capital pool pulse">
    <DashboardMetric label="Active pool" :value="poolLabel" unit="Live" tone="ok" :to="ledgerTo" />
    <DashboardMetric
    ...
    <template #chart>
      <DashboardBarChart :data="poolChartData" :empty="!hasPoolMix" />
    </template>

    <template #visuals>
      <DashboardShareBars :rows="containerRows" empty-label="No open containers" />
    </template>
  </DashboardPulseCard>
</template>
...
const ledgerTo = computed(() => ({ name: 'app-capital-ledger-page', params: withSlug() }));
const shipmentsTo = computed(() => ({ name: 'app-capital-shipments-page', params: withSlug() }));

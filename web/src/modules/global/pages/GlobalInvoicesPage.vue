<template>
  <q-page class="q-pa-md">
    ...
    <PageInitialLoader v-if="globalInvoiceStore.loading && !globalInvoiceStore.rows.length" />

    <q-banner v-else-if="globalInvoiceStore.error" class="bg-negative text-white q-mb-md" rounded>
      {{ globalInvoiceStore.error }}
    </q-banner>

    <div
      v-else-if="!globalInvoiceStore.rows.length"
    ...
    >
      ...
      <div class="text-subtitle1 text-weight-medium">No Global Invoices Found</div>
...
const filteredInvoices = computed(() => {
  ...
  return globalInvoiceStore.rows.filter((row) => {
    const matchesSearch =
      !search ||

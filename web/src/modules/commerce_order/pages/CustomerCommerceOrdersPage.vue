  ...
  <q-page class="q-pa-md commerce-orders-list-page">
    ...
    <q-card flat class="hero-surface floating-surface shadow-1 q-mb-md q-pa-md">
      <div class="row items-center justify-between">
        <div>
          <div class="text-h6 text-weight-bold text-primary">My Commerce Orders</div>
          <div class="text-caption text-grey-8">Track and manage your placed commerce orders.</div>
        </div>
      </div>
    </q-card>
    ...
    <!-- Empty State -->
    <div v-else-if="!orders.length" class="column items-center justify-center q-pa-xl text-grey-6 empty-state-block floating-surface shadow-1">
      ...
      <div class="text-caption text-grey-5">Your placed commerce orders will show up here.</div>
    </div>
...
const loadOrders = async () => {
  ...
  loading.value = true
  try {
    const res = await commerceOrderService.listCommerceOrders(authStore.tenantId, authStore.customerGroupId)
...
const onRowClick = (_evt: Event, row: CommerceOrder) => {
  ...
  void router.push(`${tenantPrefix}/shop/commerce-shop/orders/${row.id}`)
}

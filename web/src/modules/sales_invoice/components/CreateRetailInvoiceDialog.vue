      ...
      <q-card-section>
        <q-form class="q-gutter-y-md" @submit.prevent="onSubmit">
          ...
          <q-input
            ...
            label="Invoice Number / Code"
            placeholder="Leave blank to auto-generate (INV-RT-YYYYMMDD-0001)"
            outlined
            dense
...
const canSubmit = computed(() => {
  const baseCheck = Boolean(form.tenant_id && form.recipient_name.trim());
...
const resetForm = (tenantId: number | null) => {
  ...
  form.retail_billing_mode = props.initialMode || 'account';

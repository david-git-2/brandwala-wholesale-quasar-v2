...
export const useInvestorPortalStore = defineStore('investorPortal', {
  ...
  actions: {
    ...
    async fetchAllocations(tenantId: number, investorId: number, limit = 50, offset = 0) {
      this.loading = true;
      this.error = null;
      const result = await investorPortalService.listAllocations(
        tenantId,
        investorId,
        limit,
        offset,
    ...
    async fetchTransactions(tenantId: number, investorId: number, limit = 50, offset = 0) {
      this.loading = true;
      this.error = null;
      const result = await investorPortalService.listTransactions(
        tenantId,
        investorId,
        limit,
        offset,
    ...
    async fetchAllocationDetail(tenantId: number, investorId: number, globalShipmentId: number) {
      this.loading = true;
      this.error = null;
      const result = await investorPortalService.getAllocationDetail(
        tenantId,
        investorId,
        globalShipmentId,
      );

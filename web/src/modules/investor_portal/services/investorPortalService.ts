...
const listAllocations = async (tenantId: number, investorId: number, limit = 50, offset = 0) => {
  try {
    const data = await investorPortalRepository.listInvestorAllocations(
      tenantId,
      investorId,
      limit,
      offset,
...
const listTransactions = async (tenantId: number, investorId: number, limit = 50, offset = 0) => {
  try {
    const data = await investorPortalRepository.listInvestorTransactions(
      tenantId,
      investorId,
      limit,
      offset,
...
const getCapitalReport = async (
  ...
  try {
    const data = await investorPortalRepository.getInvestorCapitalReport(
      tenantId,
      investorId,
      startDate,
      endDate,
...
const getAllocationDetail = async (
  ...
  try {
    const data = await investorPortalRepository.getAllocationDetail(
      tenantId,
      investorId,
      globalShipmentId,
    );

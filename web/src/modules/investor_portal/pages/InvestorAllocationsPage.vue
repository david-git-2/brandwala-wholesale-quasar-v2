        ...
        <q-markup-table flat bordered wrap-cells class="cursor-pointer">
          ...
          <tbody>
            ...
            <tr v-for="(row, index) in allocations" :key="row.id" @click="onSelectAllocation(row)">
              ...
              <td class="text-left">
                #{{ row.global_shipment_id }} - {{ row.shipment_name ?? 'Unnamed shipment' }}
              </td>
      ...
      <q-card style="width: 600px; max-width: 95vw">
        ...
        <q-card-section class="q-pt-none" v-if="selectedAllocation">
          ...
          <div class="text-body2 text-grey-7 q-mb-md">
            Shipment ID: #{{ selectedAllocation.global_shipment_id }} · Status:
            <span class="text-capitalize text-weight-medium">{{
              selectedAllocation.shipment_status
            }}</span>
          </div>
            ...
            <div class="col-6">
              <q-card flat class="bg-grey-1 q-pa-sm">
                ...
                <div class="text-weight-bold text-primary">
                  {{ formatCurrency(selectedAllocation.invested_amount) }}
                </div>
              </q-card>
            </div>
            <div class="col-6">
          ...
          <q-card
            flat
            bordered
            class="q-pa-md bg-green-1 text-green-9"
          ...
          <q-card flat bordered class="q-pa-md bg-amber-1 text-amber-9" v-else>
            <div class="row items-center">
              ...
              <div>
                ...
                shipment is fully sold and closed.
              </div>
            </div>
          </q-card>
        </q-card-section>
...
const loadData = async () => {
  ...
  const result = await investorPortalStore.fetchAllocations(
    tenantId,
    investorId,
    limit,
    offset.value,
...
const onSelectAllocation = async (row: any) => {
  ...
  const res = await investorPortalStore.fetchAllocationDetail(
    tenantId,
    investorId,
    row.global_shipment_id,
  );

...
export const calculateItemPriceGbp = (
  ...
  roundGbp(
    toSafeNumber(priceInWebGbp) + toSafeNumber(deliveryPriceGbp) + toSafeNumber(auxiliaryPriceGbp),
  );
...
export const calculateCostingPriceGbp = (
  ...
  roundGbp(
    toSafeNumber(itemPriceGbp) + (toSafeNumber(totalWeight) / 1000) * toSafeNumber(cargoRate),
  );
...
export const calculateCostingItem = (
  ...
  const offerPriceBdt =
    file.offerPriceOverrideBdt == null
      ? calculatedOfferPriceBdt
      : toSafeNumber(file.offerPriceOverrideBdt);

        ...
        >
          ...
          <template #body-cell-offerPriceBdt="props">
            <q-td :props="props" class="preview-page__price-cell">
              {{ formatBdt(props.row.offerPriceBdtValue) }}
            </q-td>
          </template>
          ...
          <template #bottom-row>
            <q-tr class="preview-page__totals-row">
              <q-td
                ...
                :class="column.name === 'offerPriceBdt' ? 'preview-page__tone-purple' : ''"
              >
...
const columns = [
  ...
  },
  {
    ...
    classes: 'preview-page__tone-purple',
    headerClasses: 'preview-page__tone-purple',
  },
];
...
const getTotalsValue = (columnName: string) => {
  switch (columnName) {
    ...
    case 'offerPriceBdt':
      return formatBdt(totals.value.offerPriceBdt);
    default:
      return '';
  }
};
...
.preview-page__table :deep(.preview-page__tone-purple) {
  background: #f3e5f5;
  color: #6b2f7a;
}

.preview-page__table :deep(th.preview-page__tone-purple) {
  font-weight: 700;
}

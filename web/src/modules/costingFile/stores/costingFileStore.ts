...
export const useCostingFileStore = defineStore('costingFile', {
  ...
  actions: {
          ...
          if (index >= 0) {
            this.costingFileItems.splice(index, 1, {
              ...
              item_type:
                ...
                payload.itemType ??
                this.costingFileItems[index]!.item_type,
            });
          }
        }
    ...
    async updateCostingFileItemsCustomerProfit(
        ...
        if (index >= 0) {
          this.costingFileItems.splice(index, 1, {
            ...
            offer_price_override_bdt:
              result.data?.offer_price_override_bdt ??
              this.costingFileItems[index]!.offer_price_override_bdt,
            offer_price_bdt:
              result.data?.offer_price_bdt ?? this.costingFileItems[index]!.offer_price_bdt,
            updated_at: result.data?.updated_at ?? this.costingFileItems[index]!.updated_at,
          });
        }

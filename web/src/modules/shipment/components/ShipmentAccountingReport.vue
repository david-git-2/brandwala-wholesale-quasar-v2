<template>
  <div class="shipment-accounting-report">
    <!-- ── Hero Header ─────────────────────────────────── -->
        ...
        <div class="row items-center justify-between q-col-gutter-sm">
          <div class="col-12 col-sm">
            ...
            <div class="text-caption text-grey-8 q-mt-xs">
              ...
              <span v-if="refreshedAt" class="q-ml-sm">· Last refreshed: {{ formatRefreshedAt(refreshedAt) }}</span>
            </div>
          </div>
...
import type { Shipment, ShipmentItem } from 'src/modules/shipment/types'
...
const props = withDefaults(
  defineProps<{
    ...
    invoicePaidById: Record<string, number>
...
const localAccountingEntries = computed(() => {
  const tId = props.parentTenantId
  return props.accountingEntries.filter(
...
const shipmentRows = computed(() => {
  ...
  return (props.shipmentItems ?? []).map((item) => {
...
const totalInvoiceRevenueBdt = computed(() =>
  props.accountingEntries.reduce((sum, row) => sum + Number(row.total_sell_amount ?? 0), 0),
)

const totalInvoiceCogsBdt = computed(() =>
  props.accountingEntries.reduce((sum, row) => sum + Number(row.total_cost_amount ?? 0), 0),
)

const totalRealizedProfitBdt = computed(() =>
  props.accountingEntries.reduce((sum, row) => sum + Number(row.gross_profit_amount ?? 0), 0),
)

const totalSoldQuantity = computed(() =>
  props.accountingEntries.reduce((sum, row) => sum + Number(row.quantity ?? 0), 0),
)
const totalInvoicePaidBdt = computed(() =>
  Object.values(props.invoicePaidById).reduce((sum: number, value) => sum + Number(value ?? 0), 0),
)
...
const formatRefreshedAt = (val: string) => {
  try {

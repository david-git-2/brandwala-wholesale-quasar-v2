...
const selectedRow = ref<ThriftCategory | null>(null);
...
async function save() {
  ...
  try {
    if (editingId.value) {
      await thriftStore.updateCategory(editingId.value, form.value.name, form.value.description);
...
function confirmDelete(row: ThriftCategory) {

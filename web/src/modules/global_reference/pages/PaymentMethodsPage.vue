...
const openEdit = (row: PaymentMethod) => {
  if (row.is_system) { showWarningDialog('System payment methods cannot be edited.', 'Protected row'); return }
  selected.value = { ...row }; dialogOpen.value = true
}
const openDelete = (row: PaymentMethod) => {
  if (row.is_system) { showWarningDialog('System payment methods cannot be deleted.', 'Protected row'); return }

  ...
  <q-dialog v-model="localOpen" persistent>
    <q-card style="min-width: 480px; max-width: 90vw">
      ...
      <q-card-section class="q-gutter-md">
        ...
        <q-select v-model="form.scope" :options="scopeOptions" label="Scope" outlined dense emit-value map-options />
...
import { PAYMENT_METHOD_CATEGORIES, PAYMENT_METHOD_SCOPES, type PaymentMethod, type PaymentMethodCreateInput } from '../types'

const props = defineProps<{ modelValue: boolean; initialData?: PaymentMethod | null }>()
const emit = defineEmits<{ 'update:modelValue': [value: boolean]; save: [payload: PaymentMethodCreateInput & { id?: number }] }>()
...
watch(() => props.initialData, (value) => {
  form.value = value
    ? { id: value.id, code: value.code, name: value.name, category: value.category, scope: value.scope, sort_order: value.sort_order, is_active: value.is_active }
    : { id: undefined, code: '', name: '', category: 'bd_mobile_wallet', scope: 'bd', sort_order: 0, is_active: true }
}, { immediate: true })

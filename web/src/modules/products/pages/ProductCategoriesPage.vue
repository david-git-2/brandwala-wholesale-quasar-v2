  ...
  <q-page class="q-pa-md">
    <q-card flat class="q-mb-md floating-surface hero-surface shadow-1">
      <q-card-section class="q-py-sm row items-center justify-between">
        ...
        <q-btn color="primary" no-caps size="sm" class="pill-btn slim-btn" label="Add Category" @click="openCreate" />
      </q-card-section>
    </q-card>
    ...
    <q-dialog v-model="dialogOpen" persistent>
      <q-card style="min-width: 420px">
        <q-card-section class="text-h6">{{ editingId ? 'Edit Category' : 'Add Category' }}</q-card-section>
...
const load = async () => {
  const result = await productService.listProductCategories({ vendorCode: vendorCode.value })
  if (!result.success) {
...
const save = async () => {
  ...
  showSuccessNotification(editingId.value ? 'Category updated successfully.' : 'Category created successfully.')

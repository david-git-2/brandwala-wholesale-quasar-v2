...
import { formStateToPreference, preferenceToFormState } from '../utils/tenantPreferenceUtils';
...
const loadPageData = async () => {
  ...
  try {
    ...
    formState.value = preferenceToFormState(TENANT_PREFERENCE_FIELDS, tenant.value.preference);
  } catch (error) {
    console.error(error);
...
const onSave = async (nextFormState: Record<string, unknown>) => {
  ...
  formState.value = preferenceToFormState(TENANT_PREFERENCE_FIELDS, result.data?.preference);

...
export const calculateLineGrossWeightKg = (item: CostingLineItemInput): number => {
  return ((item.product_weight || 0) + (item.package_weight || 0)) * (item.ordered_quantity || 0) / 1000
}
...
export const calculateLineLandedCostBdt = (
  ...
  const rawTxRate = (items && items.length > 0)
    ? calculateRawTransactionRate(shipment, items)
    : null

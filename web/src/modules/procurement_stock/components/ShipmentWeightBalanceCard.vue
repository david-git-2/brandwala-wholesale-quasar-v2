<template>
  <q-card flat bordered class="q-pa-md shipment-weight-balance-card bg-white">
    ...
    <div class="bg-blue-1 border-light rounded-borders q-pa-sm q-mb-md">
      ...
      <div class="row q-col-gutter-sm items-center">
        ...
        <div class="col-4 text-caption text-grey-7 q-pl-xs leading-tight" style="font-size: 10px;">
          The weight you paid the cargo bill on. Click save, then apply weight balance to distribute across lines.
        </div>
      </div>
    </div>
    ...
    <div class="bg-grey-1 q-pa-sm rounded-borders border-light q-mb-md">
      <div class="text-caption text-weight-bold text-grey-9 q-mb-xs row items-center justify-between">
        <span>{{ editingBoxId !== null ? 'Edit Box Weight' : 'Manage Box Weights (Record Only)' }}</span>
      ...
      <div v-if="boxes.length && savedInvoiceWeightKg > 0" class="q-mt-sm">
        ...
        >
          ...
          <span v-if="boxVerificationMatch">
            Box total ({{ boxTotalKg.toFixed(2) }} kg) matches cargo invoice weight ({{ savedInvoiceWeightKg.toFixed(2) }} kg).
          </span>
          <span v-else>
            Box total ({{ boxTotalKg.toFixed(2) }} kg) differs from cargo invoice ({{ savedInvoiceWeightKg.toFixed(2) }} kg) by {{ Math.abs(boxTotalKg - savedInvoiceWeightKg).toFixed(2) }} kg.
          </span>
        </q-banner>
      </div>
    </div>
    ...
    <div class="row q-col-gutter-xs q-mb-md text-center">
      <div class="col-3">
        <div class="bg-grey-2 q-pa-xs rounded-borders">
          ...
          <div class="text-subtitle2 text-weight-bold text-mono" style="font-size: 12px;">{{ estimatedKg.toFixed(2) }} kg</div>
        </div>
      </div>
      <div class="col-3">
        <div class="bg-grey-2 q-pa-xs rounded-borders">
          <div class="text-caption text-grey-7" style="font-size: 10px;">Box Total</div>
          <div class="text-subtitle2 text-weight-bold text-mono" style="font-size: 12px;">{{ boxTotalKg.toFixed(2) }} kg</div>
        </div>
      </div>
      <div class="col-3">
        <div class="bg-grey-2 q-pa-xs rounded-borders">
          <div class="text-caption text-grey-7" style="font-size: 10px;">Invoice</div>
          <div class="text-subtitle2 text-weight-bold text-mono" style="font-size: 12px;">{{ actualKg.toFixed(2) }} kg</div>
        </div>
      </div>
      <div class="col-3">
        <div :class="`q-pa-xs rounded-borders ${deltaBg}`">
          <div class="text-caption text-grey-7" style="font-size: 10px;">Delta</div>
          <div class="text-subtitle2 text-weight-bold text-mono" style="font-size: 12px;">{{ deltaKg.toFixed(2) }} kg</div>
        </div>
      </div>
    </div>
    ...
    <div v-if="validationError || hasUnsavedInvoiceWeight" class="q-mb-md">
      <q-banner v-if="hasUnsavedInvoiceWeight" dense class="bg-amber-1 text-amber-10 rounded-borders q-mb-xs" style="font-size: 11px;">
        <q-icon name="info" class="q-mr-xs" />
        Unsaved cargo invoice weight — click save before applying weight balance.
      </q-banner>
      <q-banner v-if="validationError" dense class="bg-warning text-black rounded-borders" style="font-size: 11px;">
...
const applyDisabledReason = computed(() => {
  if (savedInvoiceWeightKg.value <= 0) return 'Save Cargo Invoice Weight before applying weight balance'

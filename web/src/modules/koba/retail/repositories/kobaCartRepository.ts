...
const createCart = async (tenantId: number, customerGroupId: number | null): Promise<KobaCart> => {
...
const clearCartItems = async (cartId: number): Promise<void> => {
  const { error } = await supabase.from('koba_cart_items').delete().eq('cart_id', cartId);

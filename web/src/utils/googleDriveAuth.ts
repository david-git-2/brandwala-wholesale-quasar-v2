...
export class GoogleDriveAuthRequiredError extends Error {
  constructor(message = 'Google Drive access required. Sign in with Google and allow Drive permission.') {
    super(message)
...
export function throwIfGoogleDrivePermissionDenied(status: number, body: string, context: string): void {
...
export function getDriveSyncAdminEmail(): string {
  const configured =
    import.meta.env.VITE_GOOGLE_DRIVE_ADMIN_EMAIL ||
    process.env.VITE_GOOGLE_DRIVE_ADMIN_EMAIL

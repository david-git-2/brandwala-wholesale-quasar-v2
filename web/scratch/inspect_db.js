...
async function run() {
  ...
  const { data: orderItems, error: itemsErr } = await supabase
    ...
    .select(
      'id, order_id, invoice_id, product_id, quantity, inventory_item_id, cost_bdt, sell_price_bdt, recipient_price_bdt',
    )
    .order('id', { ascending: false })
    .limit(5);
  ...
  const { data: entries, error: entriesErr } = await supabase
    ...
    .select(
      'id, type, commerce_order_item_id, commerce_invoice_id, tenant_id, inventory_item_id, product_id, quantity, total_cost_amount, status',
    )
    .eq('type', 'commerce')
    .order('id', { ascending: false })
    .limit(5);

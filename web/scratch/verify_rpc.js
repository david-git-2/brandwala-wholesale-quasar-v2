...
async function checkRPCs() {
  ...
  if (txErr) {
    console.log("-> Got error (which confirms if it exists or if parameters match):", txErr.message);
  } else {
  ...
  const { data: refreshData, error: refreshErr } = await supabase.rpc('refresh_shipment_investor_profits', {
    p_global_shipment_id: 1
  });
  if (refreshErr) {
  ...
  const { data: users, error: userErr } = await supabase.from('memberships').select('role, email').limit(20);
  if (userErr) {
  ...
  const { data: bootstrapData, error: bootstrapErr } = await supabase.rpc('get_investor_bootstrap_context', {
    p_tenant_id: 10
  });
  if (bootstrapErr) {
  ...
  const { data: detailData, error: detailErr } = await supabase.rpc('get_investor_allocation_detail', {
    p_tenant_id: 10,
    p_investor_id: 1,
    p_global_shipment_id: 1
  });

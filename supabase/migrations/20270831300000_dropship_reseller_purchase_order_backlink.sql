-- Backlink reseller purchase pricing to shop_order_items (source of truth)

begin;

create or replace function public.compute_dropship_order_reseller_purchase(p_order_id bigint)
returns table (
  reseller_unit_purchase_cost numeric,
  reseller_purchase_cost numeric,
...
create or replace function public.apply_dropship_reseller_unit_purchase(
  p_order_id bigint,
  p_unit_sell_price_amount numeric
)
returns void
language plpgsql
security definer
...
as $$
begin
  if p_unit_sell_price_amount is null or p_unit_sell_price_amount < 0 then
    raise exception 'reseller unit purchase cost must be zero or positive';
  end if;

  update public.shop_order_items soi
  set
    unit_sell_price_amount = p_unit_sell_price_amount,
    updated_at = now()
  where soi.order_id = p_order_id;

-- Update public.list_items_paginated to support recursive parent fetching for Tree view
drop function if exists public.list_items_paginated(bigint, integer, integer, text, text, text, text, text, text);
drop function if exists public.list_items_paginated(bigint, integer, integer, text, text, text, text, text, text, boolean);
...
begin
  ...
  if p_include_parents then
    ...
    from (
      ...
      item_hierarchy as (
        select
          m.id, m.tenant_id, m.parent_id, m.type, m.title, m.content, m.status, m.priority,
        ...
        select
          p.id, p.tenant_id, p.parent_id, p.type, p.title, p.content, p.status, p.priority,
      ...
      select distinct
        h.id, h.tenant_id, h.parent_id, h.type, h.title, h.content, h.status, h.priority,
        h.created_by_email, h.due_date, h.start_date, h.created_at, h.updated_at, h.archived_at,
        (
    ...
    from (
      select
        i.id, i.tenant_id, i.parent_id, i.type, i.title, i.content, i.status, i.priority,
...
grant execute on function public.list_items_paginated(bigint, integer, integer, text, text, text, text, text, text, boolean) to authenticated;

-- =========================================================
-- Migration: Add parent_id to tenants for hierarchical structure
-- =========================================================

alter table public.tenants
add column if not exists parent_id bigint references public.tenants(id) on delete set null;

-- Self-reference protection constraint
alter table public.tenants
drop constraint if exists tenants_parent_id_check;

alter table public.tenants
add constraint tenants_parent_id_check check (parent_id is null or parent_id <> id);

-- Create index on parent_id
create index if not exists tenants_parent_id_idx on public.tenants(parent_id);

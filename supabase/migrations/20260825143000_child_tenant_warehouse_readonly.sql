-- Child tenant members with global_stock view can read parent warehouse stock (read-only in UI).

create or replace function public._can_view_parent_warehouse_stock(p_parent_tenant_id bigint)
returns boolean
language sql
stable
security definer

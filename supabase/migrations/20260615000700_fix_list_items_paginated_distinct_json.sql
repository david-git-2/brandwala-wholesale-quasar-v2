-- Fix list_items_paginated distinct json comparison error
  ...
  if p_include_parents then
    ...
    from (
      ...
      distinct_hierarchy as (
        select distinct
          id, tenant_id, parent_id, type, title, content, status, priority,
          created_by_email, due_date, start_date, created_at, updated_at, archived_at
        from item_hierarchy
      )
      select
        dh.id, dh.tenant_id, dh.parent_id, dh.type, dh.title, dh.content, dh.status, dh.priority,

-- Weight balance must not overwrite received_weight (cargo invoice weight).
-- That field is set only via explicit save on the Weight Balance card.
...
drop function if exists public.apply_global_shipment_weight_balance(bigint, numeric, jsonb, numeric);

-- Migration: 20270831000090_add_proforma_status_and_atomic_issue_rpc.sql
-- Description: Add proforma_generated enum value and create atomic issue_wholesale_invoice RPC

-- 1. Add proforma_generated to global_invoice_status enum
ALTER TYPE "public"."global_invoice_status" ADD VALUE IF NOT EXISTS 'proforma_generated' BEFORE 'posted';

-- 2. Atomic Wholesale Invoice Issue RPC
...
begin
  ...
  for v_db_item in select * from public.sales_invoice_items where invoice_id = p_invoice_id loop
    ...
    if v_stock.id is not null then
      ...
      -- Deduct child tenant allocation if exists
      if exists (
        select 1 from public.global_stock_allocations
        where child_tenant_id = v_eff_tenant_id and stock_id = v_db_item.global_stock_id
      ) then

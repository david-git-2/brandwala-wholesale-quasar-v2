-- Migration: thrift sales invoice advance_amount (online COD only)
-- cod_expected = max(0, gross_cod - advance_amount). Advance is not a discount / not in PnL sell lines.
-- Depends on: 20270802000043 (latest create_thrift_sales_invoice)
...
ALTER TABLE public.thrift_sales_invoices
  ...
  ADD COLUMN IF NOT EXISTS advance_note TEXT NULL;

DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname = 'thrift_sales_invoices_advance_amount_check'
  ) THEN
    ALTER TABLE public.thrift_sales_invoices
      ADD CONSTRAINT thrift_sales_invoices_advance_amount_check
      CHECK (advance_amount >= 0);
  END IF;
END $$;

COMMENT ON COLUMN public.thrift_sales_invoices.advance_amount IS
  'Customer advance collected at create (online COD only). Deducted from cod_expected; non-refundable. Offline always 0.';
COMMENT ON COLUMN public.thrift_sales_invoices.advance_note IS
  'Optional note for advance (e.g. payment ref). Null when unused.';

-- =========================================================
-- 2. create_thrift_sales_invoice — p_advance_amount / p_advance_note
-- =========================================================
...
CREATE OR REPLACE FUNCTION public.create_thrift_sales_invoice(
  ...
  p_advance_amount NUMERIC(12,2) DEFAULT 0.00,
...
REVOKE ALL ON FUNCTION public.create_thrift_sales_invoice(
  BIGINT, TEXT, TEXT, TEXT, TIMESTAMPTZ, TEXT, TEXT, TEXT, TEXT, NUMERIC, JSONB, TEXT, TEXT, TEXT, NUMERIC, TEXT, NUMERIC, TEXT, NUMERIC, TEXT, TEXT, BIGINT, JSONB, TEXT, JSONB, NUMERIC, TEXT
) FROM PUBLIC;
...
COMMENT ON FUNCTION public.create_thrift_sales_invoice(
  ...
  'Create thrift sales invoice. Online COD: optional advance_amount reduces cod_expected (floor 0). IN_STORE rejects non-zero advance. PnL sell lines unchanged.';

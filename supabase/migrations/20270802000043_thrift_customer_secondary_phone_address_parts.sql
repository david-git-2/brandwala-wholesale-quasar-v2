-- Migration: thrift customer secondary_phone + address_parts JSONB
-- Invoice snapshots the same fields on create.
-- address = freeform street line; address_parts = { district, thana, post_code }
-- Depends on: 20270802000042
...
-- 1. Schema
-- =========================================================
ALTER TABLE public.thrift_customers
...
ALTER TABLE public.thrift_sales_invoices
  ...
  ADD COLUMN IF NOT EXISTS customer_address_parts JSONB NOT NULL DEFAULT '{}'::jsonb;

COMMENT ON COLUMN public.thrift_customers.secondary_phone IS
  'Optional alternate phone (not unique; primary phone_normalized remains upsert key).';
COMMENT ON COLUMN public.thrift_customers.address_parts IS
  'BD location parts from static district/thana/postcode catalogs: { district, thana, post_code }. Freeform street stays in address.';
COMMENT ON COLUMN public.thrift_sales_invoices.customer_secondary_phone IS
  'Snapshot of customer secondary phone at create.';
COMMENT ON COLUMN public.thrift_sales_invoices.customer_address_parts IS
  'Snapshot of address_parts at create: { district, thana, post_code }.';

-- =========================================================
-- 2. create_thrift_sales_invoice — new args
-- =========================================================
DROP FUNCTION IF EXISTS public.create_thrift_sales_invoice(
  BIGINT, TEXT, TEXT, TEXT, TIMESTAMPTZ, TEXT, TEXT, TEXT, TEXT, NUMERIC, JSONB, TEXT, TEXT, TEXT, NUMERIC, TEXT, NUMERIC, TEXT, NUMERIC, TEXT, TEXT, BIGINT, JSONB
);
...
CREATE OR REPLACE FUNCTION public.create_thrift_sales_invoice(
  ...
  p_courier_provider_id BIGINT DEFAULT NULL,
  p_meta JSONB DEFAULT '{}'::jsonb,
  p_customer_secondary_phone TEXT DEFAULT NULL,
  p_customer_address_parts JSONB DEFAULT '{}'::jsonb
)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
...
REVOKE ALL ON FUNCTION public.create_thrift_sales_invoice(
  BIGINT, TEXT, TEXT, TEXT, TIMESTAMPTZ, TEXT, TEXT, TEXT, TEXT, NUMERIC, JSONB, TEXT, TEXT, TEXT, NUMERIC, TEXT, NUMERIC, TEXT, NUMERIC, TEXT, TEXT, BIGINT, JSONB, TEXT, JSONB
) FROM PUBLIC;
...
COMMENT ON FUNCTION public.create_thrift_sales_invoice(
  ...
  'Create thrift sales invoice. Upserts customer with secondary_phone + address_parts. Online requires district/thana + address line.';

-- Deferred from 20260728202828_backfill_middleman_universal_wallet.sql (needs public.universal_wallet_ledger from 20261220000000).

-- Migration: Backfill billing_profile_wallet_ledger into universal_wallet_ledger (Phase 2)
-- Date: 2026-07-28
-- Safe on remotes where billing_profile_wallet_ledger was already retired (20270129000001)
-- before this deferred file was added to history.

begin;

do $$
begin
  if to_regclass('public.billing_profile_wallet_ledger') is null then
    raise notice 'billing_profile_wallet_ledger missing; skip middleman UWL backfill';
    return;
  end if;

  -- Dynamic SQL so the planner does not require the legacy table when it is absent.
  execute $sql$

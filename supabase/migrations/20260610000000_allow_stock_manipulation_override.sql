alter table public.products add column if not exists stock_override integer default null;
    ...
    $sql$
      with base as (
        select
          ...
          p.product_weight,
          p.package_weight,
          (p.is_available and coalesce(p.stock_override, cis.usable_quantity, 0) > 0) as is_available,
          p.created_at,
          p.updated_at,
          p.vendor_code,
          p.market_code

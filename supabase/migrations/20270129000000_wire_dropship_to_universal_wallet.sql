-- Migration: Wire dropship order flow to universal_wallet_ledger
-- Phase 1 of Wallet Unification: parallel writes to universal_wallet_ledger
-- Legacy billing_profile_wallet_ledger writes are kept until Phase 3 retirement.

begin;

-- ============================================================================
-- 1. Redefine advance_dropship_order_status
--    Forward transitions: write dropship_profit credit + revenue credit to UWL
--    Rollback to processing: delete matching UWL entries by source_id = order_no
-- ============================================================================
...
begin
  ...
  if p_target_status in ('ready_for_pickup', 'shipped', 'delivered', 'payment_received') then
    ...
    -- Calculate profit using dynamic item query (middle_man_payout_amount column dropped)
    select
    ...
    -- Legacy billing_profile_wallet_ledger write (kept until Phase 3)
    ...
    if v_order.global_invoice_id is not null then
      ...
      -- Remove legacy wallet profit entry on rollback
...
-- ============================================================================
-- 2. Redefine post_global_invoice to add parallel UWL write for dropship invoices
--    (invoice_billed debit). Legacy billing_profile_wallet_ledger write is kept.
-- ============================================================================
...
declare
  ...
  v_unit_cost numeric;
  v_billing_profile_entity_id bigint;
begin

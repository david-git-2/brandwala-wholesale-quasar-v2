-- Migration: Automatically deactivate dropship product listings when inventory reaches 0
-- Also automatically reactivate listings when inventory is restocked > 0

begin;

-- 1. Redefine submit_shop_order_from_cart to set is_active = false if dropship stock drops to <= 0
...
begin
  ...
  insert into public.shop_orders (
    tenant_id, shop_id, customer_group_id, order_no, status, order_mode_snapshot,
    recipient_name, recipient_phone, recipient_phone_secondary,
    shipping_address, shipping_district, shipping_thana,
    recipient_profile_id, merchant_billing_profile_id,
    created_by_user_id,
    cod_charge_amount, delivery_charge_amount, print_charge_amount, packing_charge_amount, discount_amount,
    is_prepaid, delivery_instructions, deduct_charges_from_margin,
    is_cod_deducted_from_margin, is_delivery_deducted_from_margin, is_print_deducted_from_margin, is_packing_deducted_from_margin
  )
  values (
    v_cart.tenant_id, v_cart.shop_id, v_cart.customer_group_id, v_order_no, v_order_status, v_shop.order_mode,
  ...
  if v_shop.shop_type = 'dropship' then
    for v_ci in select * from public.shop_cart_items where cart_id = p_cart_id loop
      ...
      -- Check remaining available stock & override quantity; deactivate listing if 0 or negative
      if v_ci.product_id is not null and v_ci.global_stock_allocation_id is not null then
...
-- 2. Redefine restock_dropship_order_on_delete trigger function to set is_active = true when restocked stock > 0

...
-- 1. Update place_koba_order function
...
-- 2. Backfill existing koba_orders total_commission
update public.koba_orders o
set total_commission = (
  select coalesce(sum(greatest(0, coalesce(oi.commission, 0) - coalesce(s.gateway_charge_flat, 20.00)) * oi.quantity), 0)
  from public.koba_order_items oi
  left join public.koba_retail_settings s on s.tenant_id = o.tenant_id

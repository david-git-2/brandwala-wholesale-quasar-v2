-- Migration: Exclude 100% stolen inventory items from stock lists
begin;
...
as $$
  with filtered as (
    ...
    where ii.tenant_id = p_tenant_id
      ...
      and (s.stolen_quantity is null or s.stolen_quantity = 0 or s.stolen_quantity < s.available_quantity)
  ),
  counted as (
...
drop function if exists public.list_global_inventory_items_with_stock(integer, integer, text, text, jsonb);

create function public.list_global_inventory_items_with_stock(
  p_page integer default 1,
  p_page_size integer default 20,
  p_sort_by text default 'id',
  p_sort_order text default 'desc',
...
grant execute on function public.list_global_inventory_items_with_stock(integer, integer, text, text, jsonb)
to authenticated;

-- PBC Workflow Redesign: Statuses, Quantities, and Parent Pull Eligibility
BEGIN;

-- 1. Add confirmed_quantity and ordered_quantity columns to product_based_costing_items if they don't exist
ALTER TABLE public.product_based_costing_items
  ADD COLUMN IF NOT EXISTS confirmed_quantity INTEGER NULL,
  ADD COLUMN IF NOT EXISTS ordered_quantity INTEGER NULL;

-- Backfill confirmed_quantity from quantity where confirmed_quantity is null
UPDATE public.product_based_costing_items
SET confirmed_quantity = quantity
WHERE confirmed_quantity IS NULL;

-- 2. Data migrate existing file statuses to new workflow values
-- pending -> pending
-- offered -> offered
-- processing -> confirmed
-- ordered -> placing_order
...
-- 3. Update RPC: upsert_pbc_backlog_from_item
-- Backlog open_quantity = confirmed_quantity - ordered_quantity (fallback quantity - delivered_quantity if confirmed is null)
...
-- 4. Harden list_child_procurement_lines
-- Costing items eligible when:
-- - pcf.status = 'ready_for_shipment'
-- - pci.status IN ('accepted', 'partial')
-- - coalesce(pci.ordered_quantity, 0) > 0
-- - pci.assigned_shipment_id IS NULL AND pci.status <> 'on_shipment'
-- - quantity returned is ordered_quantity
...
BEGIN
  ...
  (
    ...
    WHERE t.parent_id = p_parent_tenant_id
      ...
      AND pci.status IN ('accepted', 'partial')
...
-- 5. Harden add_child_line_to_parent_shipment for costing_item
...
BEGIN
  ...
  ELSIF v_source_type = 'costing_item' THEN
    ...
    IF v_costing_file.status <> 'ready_for_shipment' THEN
      RAISE EXCEPTION 'costing file must be in ready_for_shipment status';
    END IF;

    IF v_costing_item.status NOT IN ('accepted', 'partial') THEN
      RAISE EXCEPTION 'only accepted or partial costing items can be added to a shipment';
    END IF;

    IF coalesce(v_costing_item.ordered_quantity, 0) <= 0 THEN
      RAISE EXCEPTION 'costing item ordered_quantity must be greater than 0 to add to shipment';
    END IF;

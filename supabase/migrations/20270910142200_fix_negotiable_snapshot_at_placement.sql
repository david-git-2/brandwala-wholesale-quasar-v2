-- CATALOG_NEGOTIATION.md: is_negotiable_snapshot = shop.is_negotiable AND customer can_negotiate at placement.

begin;

-- Backfill vendor_catalog orders using the order's customer group + shop access.
update public.shop_orders o
set is_negotiable_snapshot = sub.negotiable
from (
  select
    o2.id,
    (
      coalesce(o2.shop_type_snapshot = 'vendor_catalog', false)
      and coalesce(
        case
          when scga.status is false or coalesce(cgp.is_active, true) = false then false
          else coalesce(scga.can_negotiate, cgp.default_can_negotiate, false)
        end,
        false
      )
    ) as negotiable
  from public.shop_orders o2
  left join public.shop_customer_group_access scga
    on scga.shop_id = o2.shop_id
    and scga.customer_group_id = o2.customer_group_id
  left join public.customer_group_shop_profiles cgp
    on cgp.customer_group_id = o2.customer_group_id
    and cgp.tenant_id = o2.tenant_id
  where o2.shop_type_snapshot = 'vendor_catalog'
) sub
where o.id = sub.id
  and o.is_negotiable_snapshot is distinct from sub.negotiable;

-- submit_shop_order_from_cart: freeze shop + customer-group negotiate permission at placement.

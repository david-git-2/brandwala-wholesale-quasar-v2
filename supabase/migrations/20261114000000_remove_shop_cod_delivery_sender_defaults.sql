-- =========================================================
-- COD/delivery + merchant sender defaults move off shops:
--   COD/delivery → courier_services
--   Sender/pickup → merchant_profiles
-- Shop keeps print/packing (+ their deduct toggles) only.
-- =========================================================

begin;

-- 1. Drop obsolete shop columns
alter table public.shops
  drop column if exists default_cod_charge_pct,
  drop column if exists default_delivery_charge_amount,
  drop column if exists deduct_cod_from_margin,
  drop column if exists deduct_delivery_from_margin,
  drop column if exists default_sender_name,
  drop column if exists default_pickup_phone,
  drop column if exists default_pickup_address,
  drop column if exists default_payout_account_type,
  drop column if exists default_payout_account_info;

-- Cart no longer snapshots shop COD/delivery deduct flags
alter table public.shop_carts
  drop column if exists deduct_cod_from_margin,
  drop column if exists deduct_delivery_from_margin;

-- 2. Redefine upsert_shop (print/packing only for dropship charge defaults)
...
-- 4. Redefine get_or_create_shop_cart
...
-- 5. Redefine submit_shop_order_from_cart
-- COD/delivery deduct flags on order default false (set later from courier on Process Order)

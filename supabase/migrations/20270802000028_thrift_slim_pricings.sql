-- Phase 22 (light): Slim thrift_pricings consumers — list price + locks only.
-- Keep cost_of_goods_sold / target_price / extra_expense_cost columns for audit;
-- stop exposing them from list RPC. Do not DROP in this migration.

begin;

comment on column public.thrift_pricings.cost_of_goods_sold is
  'DEPRECATED (P22) — do not use as sale/report COGS. Source of truth: compute_thrift_landed_unit_cost + thrift_sales_invoice_items.landed_unit_cost_at_sale. Column retained until Wave 2+ drop.';

comment on column public.thrift_pricings.target_price is
  'DEPRECATED (P22) — prefer listed_unit_price + engine suggested sell. Column retained until Wave 2+ drop.';

comment on column public.thrift_pricings.extra_expense_cost is
  'DEPRECATED (P22) — not used by costing engine. Column retained until Wave 2+ drop.';

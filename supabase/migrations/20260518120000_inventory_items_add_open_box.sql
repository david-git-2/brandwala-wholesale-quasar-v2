alter table public.inventory_items
  add column if not exists is_open_box boolean not null default false;
...
as $$
  with filtered as (
    ...
    where ii.tenant_id = p_tenant_id
      ...
      and (
        not (p_filters ? 'is_open_box')
        or ii.is_open_box = (p_filters->>'is_open_box')::boolean
      )
  ),
  counted as (
    select

-- product_based_costing_items.status was dropped in 20260729170800.
-- upsert_pbc_backlog_from_item still reads v_item.status on environments that
-- applied an older function body (trigger trg_pbc_items_auto_backlog → 42703
-- on INSERT). Recreate without the dropped column; gate backlog on quantities.

...
-- Keep latest accounting row per order_item_id before enforcing uniqueness.
with ranked as (
  select
    id,
    order_item_id,
    row_number() over (partition by order_item_id order by id desc) as rn
  from public.commerce_accounting
)
delete from public.commerce_accounting ca
using ranked r
where ca.id = r.id
  and r.rn > 1;

create unique index if not exists commerce_accounting_order_item_unique_idx
  on public.commerce_accounting (order_item_id);

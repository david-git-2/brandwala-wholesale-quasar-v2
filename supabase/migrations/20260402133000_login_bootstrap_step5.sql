-- =========================================================
-- Step 5: Post-login bootstrap RPCs
-- Return session-ready context for /app and /shop scopes
-- =========================================================
...
as $$
  with matched_member as (
    ...
    where lower(trim(cgm.email)) = lower(trim(coalesce(p_email, public.current_user_email())))
      and cgm.is_active = true
      and cg.is_active = true
      and (p_tenant_id is null or cg.tenant_id = p_tenant_id)
      and (
        p_customer_group_member_id is null
        or cgm.id = p_customer_group_member_id
      )
    order by
      case cgm.role
        when 'admin' then 1

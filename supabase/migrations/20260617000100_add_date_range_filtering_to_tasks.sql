-- Migration to add date range filtering to list_items_paginated
begin;

-- 1. Drop existing function signature (11 parameters)
drop function if exists public.list_items_paginated(bigint, integer, integer, text, text, text, text, text, text, boolean, bigint);

-- 2. Drop the new signature if it exists (14 parameters)
drop function if exists public.list_items_paginated(bigint, integer, integer, text, text, text, text, text, text, boolean, bigint, text, timestamptz, timestamptz);

-- 3. Create list_items_paginated with date range filtering
...
grant execute on function public.list_items_paginated(bigint, integer, integer, text, text, text, text, text, text, boolean, bigint, text, timestamptz, timestamptz) to authenticated;

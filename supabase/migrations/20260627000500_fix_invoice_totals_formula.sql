-- Migration: Fix Commerce Invoice Total Amount Formula to Include Wrapping and COD
begin;
...
begin
  ...
  if v_is_delivery_charge_inclusive then
    v_total_amount := greatest(0, v_subtotal + v_wrapping_charge + v_cod - v_discount_amount);
  else
    v_total_amount := greatest(0, v_subtotal + v_delivery_charge + v_wrapping_charge + v_cod - v_discount_amount);
  end if;

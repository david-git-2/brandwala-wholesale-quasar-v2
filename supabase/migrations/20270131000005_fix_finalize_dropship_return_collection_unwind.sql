-- Hotfix: finalize_dropship_return must unwind receivable when remittance posted
-- invoice_collection without a prior invoice_billed (collection-only unwind).
-- Policy: debit return_collection_reversal only — do not synthesize return_reversal.
...
-- Thin wrapper: delegates to finalize (wallet-only). No legacy ledger writes.
...
-- Backfill: already-finalized returns that have invoice_collection without invoice_billed
-- and are missing return_collection_reversal (e.g. ORD-20260730-98105).
do $$
declare
  r record;
  v_bp_id bigint;
...
begin
  for r in
    select o.id, o.tenant_id, o.order_no, o.return_ref, o.billing_profile_id, o.customer_group_id
    from public.shop_orders o
    where o.status = 'returned'
      and o.return_sub_state = 'return_finalized'
      and o.shop_type_snapshot = 'dropship'
      and exists (
        select 1 from public.universal_wallet_ledger uwl
        where uwl.tenant_id = o.tenant_id
          and uwl.source_type = 'shop_order'
          and uwl.source_id = o.id::text
          and uwl.metadata->>'transaction_type' = 'invoice_collection'
      )
      and not exists (
        select 1 from public.universal_wallet_ledger uwl
        where uwl.tenant_id = o.tenant_id
          and uwl.source_type = 'shop_order'
          and uwl.source_id = o.id::text
          and uwl.metadata->>'transaction_type' = 'invoice_billed'
      )
      and not exists (
        select 1 from public.universal_wallet_ledger uwl
        where uwl.tenant_id = o.tenant_id
          and uwl.source_type = 'shop_order'
          and uwl.source_id = o.id::text
          and uwl.metadata->>'transaction_type' = 'return_collection_reversal'
      )
  loop
    v_bp_id := r.billing_profile_id;
    if v_bp_id is null and r.customer_group_id is not null then
      select id into v_bp_id
      from public.billing_profiles
      where tenant_id = r.tenant_id
        and customer_group_id = r.customer_group_id
    ...
    select coalesce(sum(base_amount), 0) into v_collected
    from public.universal_wallet_ledger
    where tenant_id = r.tenant_id
      and source_type = 'shop_order'
      and source_id = r.id::text
      and entity_type = 'customer'
      and entity_id = v_bp_id
      and type = 'credit'
      and metadata->>'transaction_type' = 'invoice_collection';

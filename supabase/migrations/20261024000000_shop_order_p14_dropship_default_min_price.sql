-- SQL Migration to update add_to_shop_cart dropship default price to minimum sell price
begin;
  ...
  if v_shop_type in ('fixed_price', 'dropship') then
    ...
    if v_shop_type = 'dropship' then
      if coalesce(v_can_set_dropship_price, false) then
        -- Default to minimum sell price instead of default sell price
        v_customer_sell_price_amount := coalesce(p_customer_sell_price_amount, v_min_sell_price_amount);
        v_customer_sell_price_currency_id := coalesce(p_customer_sell_price_currency_id, v_min_sell_price_currency_id);

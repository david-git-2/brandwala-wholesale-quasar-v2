-- Migration: Add transactional RPC for adding commerce invoice items
begin;
...
begin
  -- 1. Fetch commerce order context
  ...
  -- 2. Fetch commerce invoice details
  ...
  -- 3. Adjust inventory stock if p_inventory_item_id is provided
  ...
  -- 4. Check if item already exists in commerce_order_items
  if p_inventory_item_id is not null then
    select id, quantity
  ...
  -- 5. Insert or update commerce_order_items
...
grant execute on function public.add_commerce_invoice_item to authenticated;
grant execute on function public.add_commerce_invoice_item to service_role;

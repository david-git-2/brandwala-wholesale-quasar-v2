-- Migration: Auto invoice creation on ready_for_pickup status for dropship orders, and draft rollback on processing
begin;

-- 1. Redefine create_dual_invoice_from_dropship_order to allow ready_for_pickup+ statuses, copy correct charges
...
-- 2. Redefine advance_dropship_order_status to automatically create invoice on ready_for_pickup+, or toggle draft status on rollback
...
begin
  ...
  if p_target_status in ('ready_for_pickup', 'shipped', 'delivered', 'payment_received') then
    ...
    if v_order.global_invoice_id is not null then
      -- Unpost invoice (put to draft / hold)
      select * into v_invoice from public.global_invoices where id = v_order.global_invoice_id;

-- Phase A (task-rto): first DELIVERED writes thrift_sales_pnl_lines + economics_closed_at.
-- Never touches payment_status / COD fields. Idempotent when already DELIVERED.
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'economics_closed_at', CASE
      WHEN v_target = 'DELIVERED' THEN COALESCE(v_invoice.economics_closed_at, v_event_at)
      ELSE v_invoice.economics_closed_at
    END
  );
END;
$$;
...
COMMENT ON FUNCTION public.update_thrift_sales_delivery_status(BIGINT, BIGINT, TEXT, TEXT) IS
  'Advance Online parcel status. First DELIVERED inserts thrift_sales_pnl_lines (shop-paid fees) and sets economics_closed_at. Never changes payment_status. RETURNED requires revert RTO.';

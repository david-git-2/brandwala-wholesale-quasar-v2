-- Catalog shop: minimum product available_units threshold for customer storefront visibility.

begin;

alter table public.shops
  add column if not exists min_available_units integer not null default 0;

alter table public.shops
  drop constraint if exists shops_min_available_units_check;

alter table public.shops
  add constraint shops_min_available_units_check check (min_available_units >= 0);
...
create or replace function public.upsert_shop(
  ...
  p_min_available_units           integer              default 0
)
...
grant execute on function public.upsert_shop(
  ...
  public.shop_type_enum, text, bigint, bigint, bigint, boolean,
  bigint, bigint, text, numeric, text, numeric, numeric, boolean, jsonb,
  boolean, boolean, text, bigint[], integer
) to authenticated;

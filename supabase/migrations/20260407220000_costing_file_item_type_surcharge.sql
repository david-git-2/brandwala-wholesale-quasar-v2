-- =========================================================
-- Costing item type surcharge
-- Add a fixed GBP 3 surcharge for watch and perfume items.
-- =========================================================

create or replace function public.calculate_costing_item_type_surcharge_gbp(
  p_item_type text
)
returns numeric
language plpgsql
...
declare
  ...
  v_item_type_surcharge_gbp numeric(12,2);
...
begin
  ...
  v_item_type_surcharge_gbp := public.calculate_costing_item_type_surcharge_gbp(new.item_type);
  v_auxiliary_price_gbp := public.calculate_costing_auxiliary_price_gbp(
    new.price_in_web_gbp,
    new.delivery_price_gbp
  )::numeric(12,2);
  v_auxiliary_price_gbp := round((coalesce(v_auxiliary_price_gbp, 0) + coalesce(v_item_type_surcharge_gbp, 0))::numeric, 2);

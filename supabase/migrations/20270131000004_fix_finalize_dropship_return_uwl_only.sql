-- Hotfix: finalize_dropship_return must not touch dropped middle_man_payout_ledger.
-- Return fee posts only to universal_wallet_ledger (billing-profile customer entity).
-- Also re-asserts mark_dropship_order_returned as a thin wrapper to finalize.

-- Migration: single RPC for thrift sales POS stock search (priced + landed)
-- Replaces 3 client round-trips with one security-definer call.
-- Landed cost: public.compute_thrift_landed_unit_cost(stock_id)
-- Depends on: 20270802000043 (or any after money-path compute)
...
BEGIN
  ...
  enriched AS (
    SELECT
      ...
      COALESCE(
        (
          SELECT ROUND(p.listed_unit_price, 2)
          FROM public.thrift_pricings p
          WHERE p.stock_id = m.id
          ORDER BY p.id DESC
          LIMIT 1
        ),
        NULL
      ) AS listed_unit_price,
  ...
  SELECT COALESCE(
    jsonb_agg(
      jsonb_build_object(
        ...
        'default_sell_price', COALESCE(
          NULLIF(e.listed_unit_price, 0),
          CASE
            WHEN e.landed_cost > 0 THEN ROUND(e.landed_cost * 1.5, 0)
            ELSE 0
          END
        ),
        'category', e.category,
...
REVOKE ALL ON FUNCTION public.search_thrift_available_stocks_for_sale(BIGINT, TEXT, TEXT, INT) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.search_thrift_available_stocks_for_sale(BIGINT, TEXT, TEXT, INT) TO authenticated;
GRANT EXECUTE ON FUNCTION public.search_thrift_available_stocks_for_sale(BIGINT, TEXT, TEXT, INT) TO service_role;

COMMENT ON FUNCTION public.search_thrift_available_stocks_for_sale(BIGINT, TEXT, TEXT, INT) IS
  'POS stock picker: one call returns available (+ matching holds) stock with landed cost and default sell price.';

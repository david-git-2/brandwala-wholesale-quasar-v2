-- Migration: middle_man_payout_ledger.customer_group_member_id must be bigint (matches customer_group_members.id)
begin;

-- Prior uuid-typed rows cannot map to customer_group_members.id
delete from public.middle_man_payout_ledger;

alter table public.middle_man_payout_ledger
  alter column customer_group_member_id type bigint
  using 0;

alter table public.middle_man_payout_ledger
  drop constraint if exists middle_man_payout_ledger_customer_group_member_id_fkey;

alter table public.middle_man_payout_ledger
  add constraint middle_man_payout_ledger_customer_group_member_id_fkey
  foreign key (customer_group_member_id) references public.customer_group_members(id);

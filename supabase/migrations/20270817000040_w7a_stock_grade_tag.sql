-- Migration W7a: Stock Grades (Column + Grain)
-- Add grade_tag_id -> tags.id. Unique grain -> (shipment_item_id, availability, location_id, grade_tag_id)

begin;

-- 1. Helper function to resolve default standard stock grade tag ID
create or replace function public.default_stock_grade_tag_id()
returns bigint
language sql
stable
security definer
...
grant execute on function public.default_stock_grade_tag_id() to authenticated;
grant execute on function public.default_stock_grade_tag_id() to service_role;

-- 2. Add column grade_tag_id to global_stocks
alter table public.global_stocks
  add column if not exists grade_tag_id bigint null constraint global_stocks_grade_tag_id_fkey references public.tags(id);

-- 3. Backfill existing global_stocks rows to standard stock grade tag
do $$
declare
  v_std_id bigint;
begin
  v_std_id := public.default_stock_grade_tag_id();
  if v_std_id is not null then
    update public.global_stocks
    set grade_tag_id = v_std_id
    where grade_tag_id is null;
  end if;
end;
$$;

-- 4. Swap unique constraint on global_stocks to include grade_tag_id
...
alter table public.global_stocks add constraint global_stocks_grain_unique
  unique (shipment_item_id, availability, location_id, grade_tag_id);

-- 5. Update finalize_global_shipment RPC to support grade_tag_id

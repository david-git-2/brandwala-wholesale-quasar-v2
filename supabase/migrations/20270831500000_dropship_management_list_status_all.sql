-- list_dropship_shop_orders_for_staff: treat p_status / p_statuses value "all" as shipped + delivered.
    ...
    and (
      case
        when p_statuses is not null and cardinality(p_statuses) > 0 then
          case
            when 'all' = any(p_statuses) then
              o.status::text in ('shipped', 'delivered')
            else
              o.status::text = any(p_statuses)
          end
        when p_status is not null and lower(trim(p_status)) = 'all' then
          o.status::text in ('shipped', 'delivered')
...
grant execute on function public.list_dropship_shop_orders_for_staff(
  bigint,
  integer,
  integer,
  text,

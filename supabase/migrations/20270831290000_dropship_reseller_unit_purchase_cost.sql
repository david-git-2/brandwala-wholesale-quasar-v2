-- Add reseller unit purchase cost to dropship settlement desk

begin;

alter table public.dropship_order_settlements
  add column if not exists reseller_unit_purchase_cost numeric(15,2) not null default 0;
...
declare
  ...
  v_default_reseller_purchase_cost numeric(15,2);
  v_default_reseller_unit_purchase_cost numeric(15,2);
...
begin
  ...
  select
    coalesce(sum(coalesce(soi.unit_sell_price_amount, 0) * soi.quantity), 0),
    case
      when coalesce(sum(soi.quantity), 0) > 0 then
        coalesce(sum(coalesce(soi.unit_sell_price_amount, 0) * soi.quantity), 0) / sum(soi.quantity)
      else 0
    end
  into v_default_reseller_purchase_cost, v_default_reseller_unit_purchase_cost
  from public.shop_order_items soi
  where soi.order_id = p_order_id;

  v_reseller_unit_purchase_cost := coalesce(
    (p_payload->>'reseller_unit_purchase_cost')::numeric,
    v_default_reseller_unit_purchase_cost
  );
  v_reseller_purchase_cost := coalesce(
    (p_payload->>'reseller_purchase_cost')::numeric,
    v_default_reseller_purchase_cost
  );
  ...
  return jsonb_build_object(
    ...
    'settlement', jsonb_build_object(
      ...
      'reseller_unit_purchase_cost', v_settlement.reseller_unit_purchase_cost,
...
begin
  ...
  select
    ...
    coalesce(sum(soi.quantity), 0)::integer
  into v_reseller_purchase_cost, v_reseller_unit_purchase_cost, v_order_item_quantity
  from public.shop_order_items soi
  where soi.order_id = p_order_id;
  ...
  return jsonb_build_object(
    ...
    'settlement', jsonb_build_object(
      ...
      'reseller_unit_purchase_cost', case
        when v_has_settlement then coalesce(nullif(v_settlement.reseller_unit_purchase_cost, 0), v_reseller_unit_purchase_cost)
        else v_reseller_unit_purchase_cost
      end,
      'reseller_purchase_cost', case
        when v_has_settlement then v_settlement.reseller_purchase_cost
        else v_reseller_purchase_cost
      end,
      'company_procurement_cost', v_company_procurement_cost,

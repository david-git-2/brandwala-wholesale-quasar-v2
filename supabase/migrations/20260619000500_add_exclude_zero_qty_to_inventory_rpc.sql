-- Migration: Add exclude_zero_qty filter to list_inventory_items_with_stock RPC
begin;
...
as $$
  with filtered as (
    ...
    where ii.tenant_id = p_tenant_id
      ...
      and (
        not (coalesce(p_filters->>'exclude_zero_qty', 'false') = 'true')
        or (
          coalesce(s.available_quantity, 0) > 0 or
          coalesce(s.reserved_quantity, 0) > 0 or
          coalesce(s.damaged_quantity, 0) > 0 or
          coalesce(s.stolen_quantity, 0) > 0 or
          coalesce(s.expired_quantity, 0) > 0 or
          coalesce(s.open_box_quantity, 0) > 0
        )
      )
  ),
  counted as (

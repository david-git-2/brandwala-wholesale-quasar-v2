-- list_global_invoice_items 400 after parent-owned invoices:
-- 1) security_invoker views were revoked from authenticated, so definer RPCs
--    that still name global_invoices* fail.
-- 2) Access used books tenant_id (parent); child membership is on issued_by.

begin;

alter view public.global_invoices set (security_invoker = false);
alter view public.global_invoice_items set (security_invoker = false);
alter view public.global_return_items set (security_invoker = false);

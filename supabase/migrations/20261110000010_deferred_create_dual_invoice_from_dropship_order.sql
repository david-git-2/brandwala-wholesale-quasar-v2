-- Deferred from 20260721153726_create_dual_invoice_from_dropship_order.sql so fresh resets create shop_orders first (20260902000600).
-- On production this re-runs as CREATE OR REPLACE / IF NOT EXISTS — safe.

-- Migration: Add create_dual_invoice_from_dropship_order RPC
-- Description: Creates a dual invoice (accounting + recipient face prices) on global_invoices for a delivered dropship shop order.
...
begin
  ...
  -- Resolve parent tenant & auth check
  ...
  -- 4. Create global_invoice record (invoice_type = dropship)
  ...
  -- 5. Copy order items to global_invoice_items with dual pricing
  for v_item in (
    ...
    -- Recipient price (face) vs Accounting sell price (middle-man buy price)
  ...
  -- 6. Outbound charges & Courier COD fee (if cod_collect_amount > 0)
  ...
  -- Add courier COD fee line charge if applicable
  ...
  -- 7. Calculate middle man payout amount
  -- Profit = Face Total - Accounting Sell Total - Margin Deductions
  v_middle_man_payout := greatest(v_face_subtotal - v_accounting_subtotal, 0);

  -- 8. Update invoice totals & link order to invoice
  ...
  -- 9. Post profit credit entry into middle_man_payout_ledger if member is present
  if v_order.customer_group_id is not null and v_middle_man_payout > 0 then
    declare
      v_member_id uuid;
      v_prev_bal numeric := 0;
    begin
      -- Locate customer group member id if registered

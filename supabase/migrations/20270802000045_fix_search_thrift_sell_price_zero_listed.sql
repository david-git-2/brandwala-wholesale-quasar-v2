-- Fix POS search sell price: treat listed_unit_price 0 as missing (match old client || fallback).
-- Depends on: 20270802000044
...
COMMENT ON FUNCTION public.search_thrift_available_stocks_for_sale(BIGINT, TEXT, TEXT, INT) IS
  'POS stock picker: listed_unit_price (null/0 skipped) else round(landed*1.5); includes holds when phone matches.';

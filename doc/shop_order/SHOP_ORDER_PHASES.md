...
**Table:** `shop_product_listings` per §7.4. UNIQUE `(shop_id, global_stock_id)`

**RPCs:**

- `list_shop_product_listings(p_shop_id)`
- `upsert_shop_product_listing(...)` — sell + min_sell money pairs; identity `id` generated by Postgres
- `list_listable_stock_for_shop(p_shop_id, …)` — assigned received sellable ATP, not already listed

Do **not** sync `is_active` from stock. No trigger on listings that reads allocations.

### Web

| Route | Page / component |
|-------|------------------|
| `/app/shop/shops/:shopId/setup` Prices tab | `ShopPricingPage.vue` |
| `/app/shop/shops/:shopId/listings/add` | `AddShopListingsPage.vue` |

### Exit

- [x] List assigned stock with sell price on fixed_price / dropship shop
- [x] `display_quantity_override` saves (dummy qty; does not flip `is_active`)
- [x] Candidates from assigned shipments + parent ATP only
...
*   Create dialog (`ShopFormDialog.vue`): **name + three types only**. Currencies defaulted (cost GBP / checkout BDT).
*   Setup page (`ShopSettingsPage.vue` at `/app/shop/shops/:id/setup`): cost / checkout currencies, vendor, pricing, quantity, dropship charges. Help ("I") lives here.
*   Letter scenarios A–F remain in Help / `shopConfigurationPresets.ts` — not create chips. See [SHOP_CREATE_TWO_STEP.md](../fix/SHOP_CREATE_TWO_STEP.md).

### Exit

- [x] Create is name + type; setup page holds the rest.
- [x] Help Dialog on setup is bilingual.
- [x] Cost / checkout currency labels (not Buy / Sell as product price).
- [x] `shop_type` locked after create.

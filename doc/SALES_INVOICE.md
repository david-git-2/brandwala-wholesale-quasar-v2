...
| **Dropship** | **As a** desk salesperson, **I want to** sell through a middle man at a face price to the end customer, **so that** the courier collects COD from the recipient, I track accounting margin separately, and settle the middle man's payout. |
...
| **P2 — Fresh backend** | `recipient_profiles` + drop-recreate invoice tables (lifecycle, retail mode) | Planned |
| **P3 — Fresh RPCs** | Create / add item / post / void / return / payment per type + `unit_cost_price` on post | Planned |
| **P4 — Wire UI** | Desk invoice pages; retail mode toggle; post/void actions; submodule guards | Planned |
